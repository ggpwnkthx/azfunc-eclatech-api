...
def bulk_validate(df, address_col, city_col=None, state_col=None, zip_col=None, auth_id=os.environ['ss_id'], auth_token=os.environ['ss_token']):
    """
    ...
    * zip_col : The name of the column containing zipcode data
    ...
    for i, row in df.iterrows():
        ...
        lookup.match='strict' # include best match even if not a valid mailable address
        batch.add(lookup)
    ...
    cleaned = pd.merge(original, data, right_index=True, left_index=True)
